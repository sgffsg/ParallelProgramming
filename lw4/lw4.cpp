...
static Pixel Average(vector<optional<Pixel>> const& v)
{
...
static Pixel GetAverageColor(Bitmap const& img, int x, int y)
{
...
static optional<Args> ParseArgs(int argc, char** argv)
{
...
static Threads CreateThreads(size_t count, function<ProcessBitmapInfo* (int)> dataCreatorFn)
{
...
static void SetNumbersDecimalPoint()
{
    locale comma_locale(locale(), new comma_numpunct());
    cout.imbue(comma_locale);
}

static void SetCoresLimit(size_t limit)
{
    SYSTEM_INFO sysinfo;
...
static GetEllapsedTimeFn StartTimer()
{
...
static int MapNumberToPriority(int number)
{
    switch (number) {
    case 0:
